...
```text
Initialize codex-sdlc for this existing repository. Explain the setup choices and show the dry run before applying changes. If the CLI is unavailable, use codex-sdlc@0.4.1 through npx.
```
...
```sh
npx --yes codex-sdlc@0.4.1 init --root /absolute/path/to/web --name example-web \
...
Only the coordinator receives `.sdlc`. The plugin skills remain in each user's Codex plugin cache. Another contributor binds their local checkout paths with `npx --yes codex-sdlc@0.4.1 configure --root <coordinator> --repo <id>=<absolute-path>` and reruns `doctor`.

Choose single-repository or multi-repository mode before starting delivery. Version 0.4.1 does not automatically convert an initialized single-repository project to multi-repository mode. Preserve existing runs before changing topology.
