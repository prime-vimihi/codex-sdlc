...
The software reads repository configuration and files needed for the requested workflow. It writes installed framework files, delivery artifacts, approvals, defects, and command evidence under `.sdlc/` in that repository. Captured command output can contain file paths, diagnostics, or other repository-derived information. Redaction removes values only for secret environment-variable names configured in the installed policy.
