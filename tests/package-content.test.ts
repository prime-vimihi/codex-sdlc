...
describe("distribution content", () => {
  ...
  test("portable plugin manifest identifies the public skills package", async () => {
    ...
    expect(codexManifest.version).toBe("0.5.0");
    ...
    expect(packageDocument.files).toContain("docs/agent-models.md");
    expect(await readFile(resolve(root, "skills/sdlc-po/SKILL.md"), "utf8")).toContain("name: sdlc-po");
  });
});
